...
export default function ClinicalRecord() {
  ...
  // --- ESTADOS PARA ROL (Lógica intacta) ---
  ...
  // --- LÓGICA DE CARGA DE ROL (INTACTA PARA RECUPERAR DATOS) ---
  const fetchRolLogs = async () => {
    try {
      ...
      // Transformación segura para la gráfica (Soporta datos viejos y nuevos)
  ...
  const fetchStatusHistory = async () => {
    // Simulado: En producción crearías una tabla 'patient_status_logs'
    // Por ahora usamos los datos básicos del paciente si existen
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex gap-2 border-b border-zinc-800">
        ...
        <button onClick={() => setActiveTab('eval')} className={`pb-2 px-4 text-sm font-medium ${activeTab === 'eval' ? 'text-white border-b-2 border-blue-500' : 'text-zinc-500'}`}>Evaluaciones Psicométricas</button>
      </div>
      ...
      {activeTab === 'eval' ? (
        ...
        // --- PESTAÑA ROL (Visualización Intacta) ---
        <div className="space-y-6 animate-in fade-in">
            ...
            <CardContent className="space-y-4">
              ...
              { (newRisk === 'medium' || newRisk === 'high') && (
                <div className="space-y-2 bg-zinc-950 p-3 rounded border border-zinc-800">
                   <input placeholder="Comentario: ¿Por qué este nivel?" value={rolComment} onChange={(e) => setRolComment(e.target.value)} className="w-full bg-zinc-900 border border-zinc-700 rounded p-2 text-sm text-white"/>
                   <textarea placeholder="Plan de Acción para esta semana" value={rolPlan} onChange={(e) => setRolPlan(e.target.value)} className="w-full bg-zinc-900 border border-zinc-700 rounded p-2 text-sm text-white h-20"/>
                </div>
              )}
          ...
          <Card className="bg-zinc-900 border-zinc-800 h-80">
            <CardContent className="h-full flex items-center justify-center pt-6">
               ...
               ) : <p className="text-zinc-500">Sin datos registrados aún.</p>}
            </CardContent>
          </Card>
          ...
          <div className="space-y-2">
            ...
            {rolLogs.map((log: any) => (
              <div key={log.id} className="bg-zinc-900 border border-zinc-800 p-3 rounded flex justify-between items-center">
                <div>
                  <span className="text-white font-bold">Sesión {log.session_number}</span>
        ...
        // --- PESTAÑA DATOS CLÍNICOS (Incluye NUEVA Gestión de Estado) ---
        <div className="space-y-6 animate-in fade-in">
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            ...
            <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4">
               ...
               <div><p className="text-xs text-zinc-500">Próxima Cuota</p><p className="text-lg text-orange-400">${patient.installment_value?.toLocaleString()}</p></div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-zinc-900 border-zinc-800 border-l-4 border-l-blue-500">
            <CardHeader>
              <CardTitle className="text-white flex items-center gap-2">
                <Activity className="w-5 h-5 text-blue-500" /> Gestión de Estado del Paciente
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <textarea 
                  ...
                  placeholder={currentStatus === 'deserter' ? "Explique por qué se marca como desertor..." : "Motivo de inactividad..."}
              ...
              {currentStatus === 'deserter' && (
                <div>
                  <label className="text-xs text-zinc-400 block mb-1 flex items-center gap-1"><Paperclip className="w-3 h-3"/> Pruebas de Comunicación (URL o Referencia)</label>
                  <input 
                    type="text" 
          ...
          {/* Notas de Evolución (Existente) */}
          <Card className="bg-zinc-900 border-zinc-800">
            <CardHeader><CardTitle className="text-white flex items-center gap-2"><FileText className="w-5 h-5 text-purple-500"/> Notas de Evolución</CardTitle></CardHeader>
            <CardContent>
              <div className="bg-zinc-950 p-4 rounded border border-zinc-800 min-h-[100px] text-zinc-400 text-sm">
                {patient.notes ? <p className="whitespace-pre-wrap">{patient.notes}</p> : <p className="italic">Sin notas clínicas registradas aún.</p>}
              </div>
            </CardContent>
          </Card>
