...
export default function ClinicalRecord() {
  ...
  const handleSaveProtocol = async () => {
    ...
    try {
      ...
      if (error) throw error;
      alert("âœ… Protocolo ClÃ­nico Guardado");
    } catch (err: any) { alert("âŒ Error: " + err.message); }
  ...
  const handleSaveSession = async () => {
    ...
    if(!num || !topic) { alert("Completa NÃºmero de SesiÃ³n y Tema"); return; }
  ...
  const handleSaveSession = async () => {
    ...
    if (!numEl || !topicEl || !numEl.value || !topicEl.value) {
      alert("⚠️ Por favor ingresa al menos el Número de Sesión y el Tema.");
      return;
    }
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <input type="number" placeholder="SesiÃ³n #" value={sessionNum} onChange={e=>setSessionNum(parseInt(e.target.value))} className="bg-zinc-950 border border-zinc-700 rounded p-2 text-white"/>
          ...
          {innerTab === 'protocol' ? (
            ...
            <div className="space-y-6 animate-in fade-in">
              <div className="bg-zinc-900/50 p-4 rounded border border-zinc-800 mb-4">
                <h3 className="text-white font-bold mb-2">I. DATOS DE FILIACIÃ“N</h3>
              ...
              <Card className="bg-zinc-900 border-zinc-800">
                <CardHeader><CardTitle className="text-white">II. EVALUACIÃ“N DEL CAPITAL MENTAL</CardTitle></CardHeader>
                <CardContent><textarea className="w-full bg-zinc-950 border border-zinc-700 rounded p-3 text-zinc-300 h-40 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Escribe aquÃ­ el anÃ¡lisis de Autoeficacia, Esperanza, Optimismo y Resiliencia..." value={protocol.assessment_capital} onChange={e=>setProtocol({...protocol, assessment_capital: e.target.value})}/></CardContent>
              </Card>

              <Card className="bg-zinc-900 border-zinc-800">
                <CardHeader><CardTitle className="text-white">III. DIAGNÃ“STICO FUNCIONAL Y PSICOMÃ‰TRICO</CardTitle></CardHeader>
              ...
              <Card className="bg-zinc-900 border-zinc-800">
                <CardHeader><CardTitle className="text-white">IV. ANÃLISIS POR EJES DE INTERVENCIÃ“N</CardTitle></CardHeader>
                <CardContent><textarea className="w-full bg-zinc-950 border border-zinc-700 rounded p-3 text-zinc-300 h-48 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Resume hallazgos de Ejes BiolÃ³gico, Cognitivo, Emocional, Ejecutivo y Relacional..." value={protocol.analysis_axes} onChange={e=>setProtocol({...protocol, analysis_axes: e.target.value})}/></CardContent>
              </Card>

              <Card className="bg-zinc-900 border-zinc-800">
                <CardHeader><CardTitle className="text-white">V. PLAN DE TRATAMIENTO PROPUESTO</CardTitle></CardHeader>
                <CardContent><textarea className="w-full bg-zinc-950 border border-zinc-700 rounded p-3 text-zinc-300 h-48 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Jerarquiza las intervenciones: EstabilizaciÃ³n, Procesamiento e IntegraciÃ³n..." value={protocol.treatment_plan} onChange={e=>setProtocol({...protocol, treatment_plan: e.target.value})}/></CardContent>
              </Card>

              <Button onClick={handleSaveProtocol} disabled={savingProtocol} className="w-full bg-blue-600 hover:bg-blue-700 py-6 text-lg">
                {savingProtocol ? 'Guardando Protocolo...' : 'ðŸ’¾ GUARDAR PROTOCOLO COMPLETO'}
              </Button>
            </div>
          ) : (
            ...
            <div className="space-y-6 animate-in fade-in">
              ...
              <div className="space-y-4"><h3 className="text-white font-bold">Historial</h3>
                {sessionHistory.length===0?<p className="text-zinc-500">Sin sesiones.</p>:sessionHistory.map((s:any)=>(
                  <Card key={s.id} className="bg-zinc-900 border-zinc-800"><CardHeader className="pb-2"><div className="flex justify-between"><span className="text-white font-bold">SesiÃ³n {s.session_number}: {s.module_topic}</span><Badge className={s.patient_status_score>=7?'bg-green-900 text-green-400':s.patient_status_score>=4?'bg-yellow-900 text-yellow-400':'bg-red-900 text-red-400'}>{s.patient_status_score}/10</Badge></div></CardHeader><CardContent className="text-sm text-zinc-300 space-y-1"><p><span className="text-zinc-500">Obs:</span> {s.clinical_observations}</p><p><span className="text-zinc-500">Comp:</span> {s.patient_commitments||'-'}</p></CardContent></Card>
                ))}
              </div>
            </div>
          )}
          
          {/* GestiÃ³n de Estado (Siempre visible al final de Datos ClÃ­nicos) */}
          <Card className="bg-zinc-900 border-zinc-800 mt-8">
             <CardHeader><CardTitle className="text-white">GestiÃ³n de Estado del Paciente</CardTitle></CardHeader>
             <CardContent className="space-y-4">
               <div className="grid grid-cols-2 gap-4">
               ...
               <textarea placeholder="Motivo / JustificaciÃ³n" value={statusComment} onChange={e=>setStatusComment(e.target.value)} className="w-full bg-zinc-950 border border-zinc-700 rounded p-2 text-white h-20"/>
