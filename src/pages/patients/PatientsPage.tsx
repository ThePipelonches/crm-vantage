...
export default function PatientsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Si es psicÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³logo, solo ve los suyos (independientemente de la pestaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±a)
      ...
      // Cargar psicÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³logos solo si es admin
      if (user?.role === 'admin') {
  ...
  const handleAssignPsychologist = async () => {
    ...
    try {
      ...
      setSelectedPsychId('');
      fetchData(); // Recargar manualmente tras ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©xito
    } catch (err: any) {
      alert('ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦Ã¢â‚¬â„¢ Error: ' + err.message);
    } finally {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      {/* Header con NavegaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-zinc-800 pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">GestiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de Pacientes</h1>
            <p className="text-zinc-400 text-sm mt-1">
              Administra asignaciones y seguimientos clÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­nicos.
            </p>
          </div>
        </div>
      ...
      {loading && patients.length === 0 ? (
        ...
        <>
          {/* PestaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±as de NavegaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n */}
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
            ...
            <TabsContent value="pending" className="mt-6">
              {user?.role === 'admin' && pendingPatients.length === 0 ? (
                <div className="text-center py-20 bg-zinc-900/50 rounded-xl border border-dashed border-zinc-800">
                  ...
                  <p className="text-zinc-500">Los leads cerrados aparecerÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡n aquÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­ para asignar psicÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³logo.</p>
                </div>
                      ...
                      <CardContent className="space-y-4">
                        ...
                        {user?.role === 'admin' && (
                          <Button onClick={() => { setSelectedPatientId(patient.id); setSelectedPsychId(''); }} className="w-full bg-blue-600 hover:bg-blue-700 text-white" size="sm">
                            <Stethoscope className="w-4 h-4 mr-2" /> Asignar PsicÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³logo
                          </Button>
                        )}
                      </CardContent>
                    </Card>
            ...
            <TabsContent value="active" className="mt-6">
              {activePatients.length === 0 ? (
                <div className="text-center py-20 text-zinc-500">No hay pacientes activos aÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºn.</div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
